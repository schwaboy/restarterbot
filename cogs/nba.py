...
class nba(commands.Cog):
    ...
    @discord.app_commands.command(name="stats", description="Player season stats")
    @discord.app_commands.describe(playername="An NBA player's name")
    async def nbastats(
        self, interaction: discord.Interaction, playername: str, games: int = 1
    ):
        # """List an NBA player's statline from the most recent game"""
        await interaction.response.defer()
        try:
            player = players.find_players_by_full_name(playername)
            assert len(player) > 0
            ...
            await interaction.followup.send(f"Player not found, {playername}")
        stats = [
            f"Recent Stats for {player[0]['full_name']} ({player_info['data'][0][20]})\n"
        ]
        info = PlayerDashboardByGameSplits(
            last_n_games=games,
            month=0,
    ...
    @discord.app_commands.command(
        name="streak",
        description="The current W/L streak and record for the last 10 games of an NBA team",
    )
    @discord.app_commands.describe(teamname="The name of an NBA team")
    async def streak(self, interaction: discord.Interaction, teamname: str):
    ...
    @discord.app_commands.command(
        name="standings", description="NBA standings by conference"
    )
